...
func FlowsAgent(ctxInfo *global.ContextInfo, cfg *beyla.Config) (*Flows, error) {
	...
	case beyla.EbpfSourceTCPLife:
		...
		fetcher, err = ebpf.NewTCPLifeFlowFetcher(cfg.NetworkFlows.Sampling, cfg.NetworkFlows.CacheMaxFlows, cfg.Filters.Network)
		if err != nil {
			return nil, err
		}
	default:
...
func flowsAgent(
	...
	mapTracer := flow.NewMapTracer(fetcher, cfg.NetworkFlows.CacheActiveTimeout, cfg.NetworkFlows.Source == beyla.EbpfSourceTCPLife)
